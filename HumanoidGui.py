...
import sys, os, time, math, random, subprocess, threading
...
class QueryStatusSignals(QtCore.QObject):
    """Carries STATUS markers from the queries_api.py reader thread to the GUI thread."""
    status = QtCore.pyqtSignal(str, str)  # (state, extra)
    finished = QtCore.pyqtSignal()
...
class AuraCore(QtWidgets.QLabel):
    def __init__(self):
        ...
        # Angle used for conical gradient animation
        self.flow_angle = 0  

        # Fixed tilt (right side higher)
        self.tilt_angle = -45
    ...
    def paintEvent(self, e):
        ...
        # Conical gradient at center, animated
        flow_grad = QtGui.QConicalGradient(cx, cy, self.flow_angle)

        # You asked for "soft white energy flow"
        # We'll use soft whites with subtle variation
        ...
        # Clip gradient to ring thickness using QPainterPath
        ...
        inner_path.addEllipse(ellipse_rect.adjusted(6, 6, -6, -6))  # ring thickness

        ring_path = outer_path.subtracted(inner_path)

        p.save()
        p.translate(cx, cy)
        p.rotate(self.tilt_angle)  # FIXED tilt
        p.translate(-cx, -cy)
...
""")
    ...
    def center_controls(self):
        ...
        self.btn_listen = GlowButton("Ask Question")
    ...
    def run_queries(self):
        ...
        try:
            ...
            # No "Listening..." on click -- the bot speaks the welcome first.
            self.status_label.clear()

            # Force UTF-8 stdout in the child so its emoji log lines don't crash
            # it when we capture output through a pipe (Windows defaults to cp1252).
            env = dict(os.environ, PYTHONIOENCODING="utf-8")

            # run queries.py in a separate Python process, capturing its output
            # so we can show "Listening..." on every recording turn.
            ...
            # Bridge STATUS lines from the reader thread to the GUI thread.
            self._query_signals = QueryStatusSignals()
            self._query_signals.status.connect(self._on_query_status)
            self._query_signals.finished.connect(self._on_query_finished)
            self._query_proc = proc
            ...
            def reader():
                for line in proc.stdout:
                    ...
                    if line.startswith("STATUS:"):
                        ...
                        self._query_signals.status.emit(state, extra)
                self._query_signals.finished.emit()

            self._query_thread = threading.Thread(target=reader, daemon=True)
    ...
    def _on_query_status(self, state, extra):
        """Show 'Listening...' whenever the bot is recording; clear otherwise."""
        if state == "RECORDING":
            self.status_label.setText("Listening...")
        else:
            # SPEAKING (welcome/answers), RECORDED, TRANSCRIBING, THINKING -> no text.
            self.status_label.clear()

    def _on_query_finished(self):
        """Session ended (user said an exit word) -> clear the text."""
        self.status_label.clear()
