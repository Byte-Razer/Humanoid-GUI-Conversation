...
class Config:
    ...
    # None = auto-detect the system default microphone (recommended).
    # Set to a specific integer only if you want to force a particular input device.
    MIC_DEVICE_ID = None
    DEFAULT_DURATION = 8
    SAMPLE_RATE = 16000
...
# ====== GUI STATUS BRIDGE ======
def emit_status(state: str, extra: str = ""):
    """Print a machine-readable status marker for the GUI to parse.

    The GUI (HumanoidGui.py) reads stdout and looks for lines that start with
    'STATUS:'. Format: STATUS:<STATE>[:<extra>]. Flushed immediately so the GUI
    updates in real time. Harmless when run standalone from a terminal.
    """
    line = f"STATUS:{state}" + (f":{extra}" if extra != "" else "")
    print(line, flush=True)


# ====== HELPER FUNCTIONS ======
def resolve_input_device(device_id: Optional[int] = None) -> Optional[int]:
    """Return a valid microphone (input) device index.

    If device_id is None, use the system default input device. If that is
    unavailable, fall back to the first device that has input channels.
    Returns None only if no input-capable device can be found.
    """
    try:
    ...
    def has_input(idx: int) -> bool:
        return 0 <= idx < len(devices) and devices[idx].get('max_input_channels', 0) > 0

    # Explicitly requested device: honor it only if it can actually record.
    if device_id is not None:
        if has_input(device_id):
            return device_id
        logger.warning(
            f"Configured device {device_id} "
            f"('{devices[device_id]['name'] if 0 <= device_id < len(devices) else 'unknown'}') "
            f"has no input channels; auto-detecting instead."
        )

    # Try the system default input device.
    try:
        default_input = sd.default.device[0]
        if isinstance(default_input, int) and has_input(default_input):
            logger.info(f"Using default input device {default_input}: {devices[default_input]['name']}")
            return default_input
    except Exception:
        pass

    # Fall back to the first input-capable device.
    for i, device in enumerate(devices):
        if device.get('max_input_channels', 0) > 0:
            logger.info(f"Falling back to input device {i}: {device['name']}")
            return i

    logger.error("No microphone (input-capable) device found")
    return None
...
def validate_environment():
    ...
    # Check that a usable microphone (input device) is available
    try:
        devices = sd.query_devices()
        mic_id = resolve_input_device(Config.MIC_DEVICE_ID)
        if mic_id is None:
            logger.warning("No microphone found. Available devices:")
            for i, device in enumerate(devices):
                logger.info(f"  {i}: {device['name']} "
                            f"({device.get('max_input_channels', 0)} in, "
                            f"{device.get('max_output_channels', 0)} out)")
            errors.append("No microphone (input-capable) device found")
        else:
            logger.info(f"Microphone check passed: device {mic_id} "
                        f"('{devices[mic_id]['name']}')")
    except Exception as e:
...
def record_audio(
    ...
    try:
        # Resolve to a valid input-capable device (auto-detect if needed)
        device_id = resolve_input_device(device_id)
        if device_id is None:
            logger.error("No usable microphone device found")
            return None
        ...
        emit_status("RECORDING", str(duration))
    ...
    while True:
        ...
        try:
            ...
            if not should_continue:
                # User wants to exit
                goodbye_msg = "Thank you for using USGS Voice Assistant. Goodbye!"
                print(f"\n👋 {goodbye_msg}")
                speak(goodbye_msg)
...
def main():
    ...
    print("=" * 50)
    print("🎓 USGS SCHOOL VOICE ASSISTANT")
    print("=" * 50)
